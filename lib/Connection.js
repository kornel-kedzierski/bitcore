...
Connection.prototype.handleMessage = function(message) {
  ...
  try {
    ...
    case 'version':
      ...
      this.sendVer = Math.min(message.version, parseInt(this.network.protocolVersion || PROTOCOL_VERSION));
      if (message.version < 209) {
        this.recvVer = Math.min(message.version, parseInt(this.network.protocolVersion || PROTOCOL_VERSION));
      } else {
...
Connection.prototype.sendVersion = function () {
  ...
  var put = new Put();
  put.word32le(parseInt(this.network.protocolVersion || PROTOCOL_VERSION)); // version
...
Connection.prototype.parseMessage = function (command, payload) {
  ...
  case 'block':
    var block = new Block();
    block.hashFunction = this.network.hashFunction || '';
    block.parse(parser);
